...
function subsetSumsMetaTesting(n=12) {
    let sum = 0;
    let factorials = Array(n + 1).fill().map((_, i) => factorial(i));

    for (let k = 2; k <= Math.floor(n/2); k++) {
        sum += ((k-1) * factorials[n]) / (2 * factorials[n-2*k] * factorials[k] * factorials[k + 1]);
        // Thanks to https://math.stackexchange.com/a/3256097 the code could be reduced to run fast!
    }
...
module.exports = { subsetSumsMetaTesting }