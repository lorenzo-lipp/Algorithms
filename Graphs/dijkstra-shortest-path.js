function dijkstra(start, end, graph) {
    ...
    while (!ipq.isEmpty()) {
        let [key, value] = [ipq.peekMinKeyIndex(), ipq.poolMinValue()];

        for (let [node, weight] of graph[key]) {
            if (dist[node] > value + weight) {
                if (!ipq.contains(node)) ipq.insert(node, value + weight);
                else ipq.decreaseKey(node, value + weight);
                prev[node] = key;
                dist[node] = value + weight;
            }
        }
    }
    ...
    for (let i = prev[end]; i !== null; i = prev[i]) path.push(i);
