...
function getSegmentPoints(segment) {
    ...
    [[_, p1.ggbName, p2.ggbName, polygon]] = [...getCommandString(segment).matchAll(/\((.+), (.+), (.+)\)/gi)];
...
function highlightSegments(s1, s2, bool) { 
    if (bool) {
        setColor(s1.name, 125, 200, 0);
        setColor(s2.name, 125, 200, 0);
    } else {
        let s1Color = getColor(s1.polygon).slice(1);
        let s2Color = getColor(s2.polygon).slice(1);
        setColor(s1.name, ...[parseInt(s1Color.slice(0, 2), 16), parseInt(s1Color.slice(2, 4), 16), parseInt(s1Color.slice(4), 16)]);
        setColor(s2.name, ...[parseInt(s2Color.slice(0, 2), 16), parseInt(s2Color.slice(2, 4), 16), parseInt(s2Color.slice(4), 16)]);
    }    
}
...
async function testNeighbors(neighbors, points, pointsIndex, testedSegments, delay) {
    for (let i = 0; i < neighbors.length - 1; i++) {
        ...
        if (!s1 || !s2 || s1.polygon === s2.polygon) continue;
...
function addSegment(point, points, activeSegments) {
    ...
    let segmentX = sweepLineIntersection(segment);

    for (let i = 0; i < activeSegments.length; i++) {
        let activeSegmentX = sweepLineIntersection(activeSegments[i]);
